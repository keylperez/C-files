...
int main(){
	...
	char letter; //varaible for inputed letter
	...
	do{	//puts asterisk in placeholder array depending on size of word
		placeholder[i]='*';
		++i;
	} while(i < size);
//	printf("%d", size);
	welcomeScreen();
		do {	//execute loop while lives is greater than 0
...
//			printf("\nvalue of word & placholder: %s & %s", word, placeholder); //debug purposes
			...
			if (YoN == 0){	//if YoN is wrong, minues 1 life, else continue
				--lives;
			}
//			printf("\nvalue of YoN: %d\n", YoN);	//debug purposes
			win = winTest(word, placeholder, size);	//return value either 1(wint) or 0(not yet)
			if (win == 1){	//if user has won, go to congratulations screen
...
//	printf("\nValue of character: %c", letter);
	printf("\n\n\tInput a letter: ");
	do{	//tests if input is small capital letter
		letter = getche();
...
int letterTest(char letter, char word[], char placeholder[], int size){	//replaces the correct inputed letter to placeholder array
	int i = 0, y = 0;
	do {
		if (word[i] == letter){	//checks if inputed letter matches any element if arrat
...
int winTest(char word[], char placeholder[], int size){	//tests if user has guessed all the letters/elements of array
	int i, correct = 0;
	for(i = 0; i < size; i++){
		if(word[i] == placeholder[i]){	//checks every element of both arrays
