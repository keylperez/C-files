...
void depositItem(Locker *L, char studID[], ItemDets newItem)
{
	if (strcmp(L->owner.studID, studID) == 0)
	{
		...
		ItemList temp, *trav;
		ItemList *head = &L->IL;
		for (trav = head; *trav != NULL && (*trav)->item.weight < newItem.weight; trav = &(*trav)->nextItem)
		{
		}

		temp = (ItemList)malloc(sizeof(ItemNode));
		if (temp != NULL)
		{
			strcpy(temp->item.itemName, newItem.itemName);
			temp->item.weight = newItem.weight;
			temp->nextItem = *trav;
			*trav = temp;
		}
	}
...
ItemList getHeavyItems(Locker *L, float limit)
{
	...
	ItemList newList, *trav;
	ItemList *head = &L->IL;
	newList = (ItemList)malloc(sizeof(ItemNode));
	for (trav = head; *trav != NULL;)
	{
		if ((*trav)->item.weight > limit)
		{
			// ItemList newList = (ItemList)malloc(sizeof(ItemNode));
			if (newList != NULL)
			{
				strcpy(newList->item.itemName, (*trav)->item.itemName);
				newList->item.weight = (*trav)->item.weight;
				newList->nextItem = NULL;
//				newList->nextItem = &newList;
				*trav = newList->nextItem;
